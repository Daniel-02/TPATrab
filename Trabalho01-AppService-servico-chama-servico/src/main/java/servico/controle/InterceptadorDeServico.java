...
public class InterceptadorDeServico implements MethodInterceptor {
	 ...
	 * para o objeto MethodInterceptor registrado quando um método interceptado
	 * é executado. Ele pode ser utilizado para invocar o método original, ou
	 * chamar o mesmo método sobre um objeto diferente do mesmo tipo.
	 * 
	 */
	...
	public Object intercept(Object objeto, Method metodo, Object[] args, MethodProxy metodoDoProxy) throws Throwable {
		try {
			...
			System.out.println("\nOcorreu uma exceção derivada de " + "RuntimeException. O método " + metodo.getName()
			...
			if (metodo.isAnnotationPresent(anotacao.RollbackFor.class)) {
				...
				for (Class<?> classeDeExcecao : classesDeExcecao) {
					if (classeDeExcecao.isInstance(e)) {
						...
						System.out.println("\nO método " + metodo.getName()
								+ " está anotado para RollBack. O Rollback acabou " + "de acontecer.");
				...
				if (!achou) {
					System.out.println(
							"\nO método " + metodo.getName() + " está anotado para RollBack, mas a exceção que "
									+ "ocorreu não consta da lista informada na anotação - " + classesDeExcecao);
				...
				System.out.println("\nOcorreu uma exceção derivada de " + "exception. O método " + metodo.getName()
						+ " não " + "está anotado para RollBack. Acabou de ocorrer um " + "commit.");
			}
