			...
			{	case 1:
				{
					...
					Produto produto = new Produto(nome, descricao, lanceMinimo, 
							                      Util.strToCalendar(dataCadastro));

					long numero = 0;
					try
					{	
						numero = produtoAppService.inclui(produto);
					}
				...
				{	int resposta = Console.readInt('\n' + 
					...
					{	try
						...
						{	System.out.println('\n' + "Este produro possui lances, logo não pode ser removido");
						}
						catch(ViolacaoDeConstraintDesconhecidaException e)
						{	System.out.println('\n' + e.getMessage());
						}
