...
public class FabricaDeServico {
	...
	public static <T> T getServico(Class<T> tipo) {
		...
		try {
			...
			// O proxy deve ainda chamar o método intercept() da classe
			// interceptadora,
			// isto é, da classe InterceptadorDeServico (classe callback).

			// Enhancer enhancer = new Enhancer();
			// enhancer.setSuperclass(classeDoServico); // Superclasse do
			// Servico
			// enhancer.setCallback(new InterceptadorDeServico()); //
			// Interceptador do Servico
