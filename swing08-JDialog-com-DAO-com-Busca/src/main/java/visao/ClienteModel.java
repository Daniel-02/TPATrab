...
public class ClienteModel extends AbstractTableModel {
		...
		if (!cache.containsKey(rowIndex)) {
			if (cache.size() > (NUMERO_DE_LINHAS_POR_PAGINA * 3)) {
				...
				if (rowIndex >= rowIndexAnterior) {
					List<Cliente> resultados = clienteService.recuperaPeloNome(nomeCliente,
					...
					List<Cliente> resultados = clienteService.recuperaPeloNome(nomeCliente, inicio,
				...
				if (rowIndex >= rowIndexAnterior) {
					List<Cliente> resultados = clienteService.recuperaPeloNome(nomeCliente, rowIndex,
							NUMERO_DE_LINHAS_POR_PAGINA * 2);
	...
	public void setValueAt(Object obj, int r, int c) {
		...
		if (c == COLUNA_NOME)
			umCliente.setNome((String) obj);
		if (c == COLUNA_SEXO)
			umCliente.setSexo((String) obj);
		...
		if (c == COLUNA_NEWS_LETTER)
			umCliente.setNewsLetter((Boolean) obj);
